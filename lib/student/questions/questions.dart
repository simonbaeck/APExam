...
class _QuestionsScreenState extends State<QuestionsScreen> with WidgetsBindingObserver {
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    onPressed: antwoorden.where((e) => e.studentId == widget.currentStudentId).length == 3 ? () {
                      addAnswersToDatabase(examContext: context);
                    } : null,
                    style: ButtonStyle(
  ...
  Future addAnswersToDatabase({ required BuildContext examContext }) async {
    if (seconds > 0) {
      showDialog(
          ...
          builder: (context) {
            return AlertDialog(
              title: Text('Examen indienen'),
              content: Text('Ben je zeker dat je je examen wil indienen?'),
              actions: <Widget>[
                TextButton(
                  onPressed: () {
